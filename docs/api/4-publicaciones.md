...
- `Mascota` **no** viaja como objeto anidado: viaja **desagregada en campos planos** con prefijo (`nombreMascota`, `tipoMascota`, `estadoMascota`, `urlFoto`), más el id en `idMascota`. ⚠️ **Corregido el 23/08/2026**: esta ficha decía que el `id` de la mascota no se exponía; eso era cierto al 31/07, antes de la integración con el frontend. Hoy `idMascota` sí viaja, y el formulario de edición depende de él para armar el `PUT /mascotas/{id}` (`src/app/models/publicacion.ts`).
